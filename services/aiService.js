...
async function curateResources(searchData, subject) {
  ...
  try {
    ...
    const summarizedContext = limitedResults.map(r => ({
     ...
     // stringifying this to avoid unescaped { } issues
    const summarizedContextStr = JSON.stringify(summarizedContext, null, 2);

    // prompt
      const prompt = ChatPromptTemplate.fromMessages([
        SystemMessagePromptTemplate.fromTemplate(
          `You are an expert educator who curates high-quality learning resources. 
      ...
      Analyze and curate exactly 5 of the most valuable and high-quality free learning resources for ${subject}.
       Context:
        ${summarizedContext}
         Requirements:
      ...
      const response = await chain.invoke({
        subject,
        summarizedContext:summarizedContextStr
      });
...
async function generatePlan(subject, userId, examDate) {
  ...
  try {
      ...
      const prompt = ChatPromptTemplate.fromMessages([
        ...
        HumanMessagePromptTemplate.fromTemplate(`
      Create a detailed study plan for ${subject} with ${daysUntilExam} days until the exam on ${examDate}.
    ...
    // depending on return shape:
    const parsedPlan = await parser.parse(response.content || "{}");
